...
/*== <操作手順>														==*/
/*== 1.[change]をクリックし、保存先ファイルを指定します。       		==*/
/*== 2.接続するSI-40LA2のIPaddressとポート番号を入力します。			==*/
/*== 3.[Connect]をクリックし、SI-40LA2に接続します。					==*/
/*== 4.SI-40LA2からのデータを受信すると指定したファイルに書込まれ		==*/
/*==   カウンターに受信バイト数が表示されます。							==*/
/*== 5.[Disconnect]をクリックし、SI-40LA2と切断ます。					==*/
/*==   拡張子.40dのファイルができますのでお持ちのテキストエディタで		==*/
/*==   確認してください。												==*/
/*== <注意>															==*/
/*== 	マルチスレッドにて作成されています。							==*/
/*====================================================================*/
...
BOOL ChangeSelectUnit(HWND hDlg, WPARAM wParam)
{
	...
	{
		// 表示するリスト
		SendMessage(hwndComb, CB_ADDSTRING, 0, (LPARAM)"1号機");
		SendMessage(hwndComb, CB_ADDSTRING, 0, (LPARAM)"2号機");
		SendMessage(hwndComb, CB_ADDSTRING, 0, (LPARAM)"5号機");
		SendMessage(hwndComb, CB_ADDSTRING, 0, (LPARAM)"6号機");
	}
...
BOOL SwitchFiles(HWND hDlg)
{
	...
	{
		// ビームの記録
		strftime((char*)datetime, 80, "\\BeamRecode\\BeamRecode_1_%Y%m%d%H%M%S.txt", tm);
	...
	{
		// 生産レポート
		strftime((char*)datetime, 80, "\\DayReport\\DayReport_1_%Y%m%d%H%M%S.txt", tm);
...
BOOL WmInitDialog( HWND hDlg, WPARAM wParam, LPARAM lParam )
{
	...
	SetDlgItemText(hDlg, IDC_IPADDRESS, (LPCSTR)"169.254.59.13");
